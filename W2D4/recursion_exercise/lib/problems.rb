...
# You can assume the exponent is never negative.
#
...
def sum_array(array)
    ...
    return array[0] + sum_array(array[1, array.length - 1])
end
...
def flatten(data)
    ...
    flattened = []
    (0...data.length).each { |i| flattened = flattened.concat(flatten(data[i])) }
    flattened
end